...
helper fetchFromTable => sub { my ($self, $table, $sessionid, $where)=@_;
	...
	{	my  %session;
		...
		{	$table = 'trials';
			@cols=qw/id idgroup name codename infotext global_state fulltext sponsor phase/;
...
helper performBooking => sub { my ($self, $piz, $dcid, $text)=@_;
    my $r= system("cd /Users/Shared/bin/BookAugDateCmd; /Library/Internet\\ Plug-Ins/JavaAppletPlugin.plugin/Contents/Home/bin/java -jar /Users/Shared/bin/BookAugDateCmd/BookAugDateCmd.jar 10.210.21.10 augdb2 xxx $piz $dcid");
	if(!$r)
	{	my $dbh_dc = DBI->connect("dbi:JDBC:hostname=localhost:9003;url=jdbc:sapdb://10.210.21.10:7210/augdb", 'daniel', 'xxx') || warn "Database connection not made: $DBI::errstr";
